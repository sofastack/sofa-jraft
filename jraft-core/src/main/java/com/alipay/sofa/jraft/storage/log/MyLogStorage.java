 ...
 * @Description:
 */
public class MyLogStorage implements LogStorage, Describer {
...
//    private final boolean openStatistics;
    ...
    private ArrayDeque<Pair<byte[],byte[]>> datalogEntries ;
    private ArrayDeque<Pair<byte[],byte[]>> conflogEntries ;
...
//    private DebugStatistics statistics;
    ...
    public MyLogStorage(final String path, final RaftOptions raftOptions) {
        super();
        this.path = path;
...
//                this.db.put(this.defaultHandle, this.writeOptions, getKeyBytes(logIndex), newValueBytes);
    ...
    private void addData(byte[] keyBytes, byte[] newValueBytes) {
