...
public class Replicator implements ThreadId.OnError {
    ...
    void installSnapshot() {
        ...
        try {
            Requires.requireTrue(this.reader == null,
                "Replicator %s already has a snapshot reader, current state is %s", this.toString(),
                getState());
    ...
    public static ThreadId start(final ReplicatorOptions opts, final RaftOptions raftOptions) {
        ...
        if (!r.rpcService.connect(opts.getPeerId().getEndpoint())) {
            LOG.error("Fail to init sending channel to {}, groupId {}.", opts.getPeerId(), opts.getGroupId());
        ...
        LOG.info("Replicator={} is started", r);
    ...
    private static boolean onAppendEntriesReturned(final ThreadId id, final Inflight inflight, final Status status,
        ...
        if (response.getTerm() != r.options.getTerm()) {
            r.resetInflights();
            r.setState(State.Probe);
            LOG.error("Fail, response term {} dismatch, expect term {}, peer {}, group id{}", response.getTerm(),
