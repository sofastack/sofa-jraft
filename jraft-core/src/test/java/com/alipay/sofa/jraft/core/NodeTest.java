...
public class NodeTest {
        ...
        // start with checksum validation
        {
            ...
            for (final PeerId peer : peers) {
                assertTrue(cluster.start(peer.getEndpoint(), false, 300, true, null, raftOptions,
                    ElectionPriorityType.NOT_SUPPORT));
            }
    ...
    public void testReadIndex() throws Exception {
        ...
        final TestCluster cluster = new TestCluster("unittest", this.dataPath, peers);
        for (final PeerId peer : peers) {
            assertTrue(cluster.start(peer.getEndpoint(), false, 300, true, ElectionPriorityType.NOT_SUPPORT));
        }
    ...
    public void testJoinNodes() throws Exception {
        ...
        // start peer1
        assertTrue(cluster.start(peer1.getEndpoint(), true, 300, ElectionPriorityType.NOT_SUPPORT));
        // add peer1
        CountDownLatch latch = new CountDownLatch(1);
        peers.add(peer1);
        ...
        Thread.sleep(2000);
        assertTrue(cluster.start(peer2.getEndpoint(), true, 300, ElectionPriorityType.NOT_SUPPORT));
    ...
    public void testSetPeer2() throws Exception {
        ...
        assertTrue(cluster.start(followerAddr1, true, 300, ElectionPriorityType.NOT_SUPPORT));
        LOG.info("start follower {}", followerAddr2);
        assertTrue(cluster.start(followerAddr2, true, 300, ElectionPriorityType.NOT_SUPPORT));
    ...
    public void testInstallLargeSnapshotWithThrottle() throws Exception {
        ...
        for (int i = 0; i < peers.size() - 1; i++) {
            ...
            final boolean started = cluster.start(peer.getEndpoint(), false, 200, false, ElectionPriorityType.NOT_SUPPORT);
            assertTrue(started);
        }
        cluster.waitLeader();
        // get leader
        ...
        final boolean started = cluster.start(newPeer.getEndpoint(), true, 300, false, snapshotThrottle, ElectionPriorityType.NOT_SUPPORT);
        assertTrue(started);
    ...
    public void testInstallLargeSnapshot() throws Exception {
        ...
        final boolean started = cluster.start(newPeer.getEndpoint(), true, 300, false, null, raftOptions, ElectionPriorityType.NOT_SUPPORT);
        assertTrue(started);
    ...
    public void testInstallSnapshot() throws Exception {
        ...
        //restart follower.
        cluster.clean(followerAddr);
        assertTrue(cluster.start(followerAddr, true, 300, ElectionPriorityType.NOT_SUPPORT));
    ...
    public void testLeaderTransferBeforeLogIsCompleted() throws Exception {
        ...
        for (final PeerId peer : peers) {
            assertTrue(cluster.start(peer.getEndpoint(), false, 1, ElectionPriorityType.NOT_SUPPORT));
        }
    ...
    public void testChangePeers() throws Exception {
        ...
        for (int i = 1; i < 10; i++) {
            ...
            assertTrue(cluster.start(peer.getEndpoint(), true, 300, ElectionPriorityType.NOT_SUPPORT));
        }
        for (int i = 0; i < 9; i++) {
            cluster.waitLeader();
            leader = cluster.getLeader();
    ...
    public void testChangePeersChaosWithSnapshot() throws Exception {
        ...
        assertTrue(cluster.start(peers.get(0).getEndpoint(), false, 2, ElectionPriorityType.NOT_SUPPORT));
        // start other peers
        for (int i = 1; i < 10; i++) {
    ...
    public void testChangePeersChaosWithoutSnapshot() throws Exception {
        ...
        peers.add(new PeerId("127.0.0.1", TestUtils.INIT_PORT));
        final TestCluster cluster = new TestCluster("unittest", this.dataPath, peers, 1000);
        assertTrue(cluster.start(peers.get(0).getEndpoint(), false, 100000, ElectionPriorityType.NOT_SUPPORT));
        // start other peers
        for (int i = 1; i < 10; i++) {
            final PeerId peer = new PeerId("127.0.0.1", TestUtils.INIT_PORT + i);
            peers.add(peer);
            assertTrue(cluster.start(peer.getEndpoint(), true, 10000, ElectionPriorityType.NOT_SUPPORT));
        }
    ...
    public void testChangePeersChaosApplyTasks() throws Exception {
        ...
        for (int i = 1; i < 10; i++) {
            final PeerId peer = new PeerId("127.0.0.1", TestUtils.INIT_PORT + i);
            peers.add(peer);
            assertTrue(cluster.start(peer.getEndpoint(), true, 100000, ElectionPriorityType.NOT_SUPPORT));
        }
